...
import { normalizeConnectionHandles } from "@/lib/canvas/canvas-node-ports";
...
export type CanvasAgentOp =
    ...
    | { type: "connect_nodes"; id?: string; fromNodeId: string; toNodeId: string; fromPortId?: string; toPortId?: string }
    ...
    (Array.isArray(ops) ? ops : []).forEach((op, index) => {
        ...
        if (op.type === "connect_nodes") {
            ...
            const normalized = normalizeConnectionHandles(
                { nodeId: op.fromNodeId, handleType: "source", portId: op.fromPortId },
                { nodeId: op.toNodeId, handleType: "target", portId: op.toPortId },
                nodes,
                connections,
            );
            if (!normalized) return;
            const exists = connections.some((conn) => conn.fromNodeId === normalized.fromNodeId && conn.toNodeId === normalized.toNodeId && conn.fromPortId === normalized.fromPortId && conn.toPortId === normalized.toPortId);
            if (!exists) connections = [...connections, { id: op.id || nanoid(), ...normalized }];
        }
