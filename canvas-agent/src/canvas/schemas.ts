...
export const canvasOpSchema = z.discriminatedUnion("type", [
    ...
    z.object({ type: z.literal("connect_nodes"), id: z.string().optional(), fromNodeId: z.string(), toNodeId: z.string(), fromPortId: z.string().optional(), toPortId: z.string().optional() }).passthrough(),
...
export const toolInputSchemas = {
    ...
    canvas_connect_nodes: z.object({ connections: z.array(z.object({ fromNodeId: z.string(), toNodeId: z.string(), fromPortId: z.string().optional(), toPortId: z.string().optional() })).min(1) }),
    ...
    generation_get_status: z.object({ scope: z.literal("canvas").optional(), nodeIds: z.array(z.string()).optional(), limit: z.number().optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    site_navigate: "跳转网站页面。path 可为 /（自动打开画布）、/canvas（我的画布）、/canvas/:id（指定画布）、/prompts（提示词库）、/assets（我的素材）、/config（配置）。操作画布前若不在画布页，先用本工具打开画布。",
    ...
    generation_get_status: "查询当前活动画布的生成任务状态；可用 nodeIds 查询指定画布节点。",
