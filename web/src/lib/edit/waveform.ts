...
export function waveformPointAt(seconds: number, audioSeconds: number, loop: boolean, points: number): number {
    if (points <= 0 || !(audioSeconds > 0)) return -1;
    const wrapped = loop ? ((seconds % audioSeconds) + audioSeconds) % audioSeconds : seconds;
    const index = Math.floor((wrapped / audioSeconds) * points);
 ...
 */
export function waveformColumns(peaks: Float32Array, troughs: Float32Array, columns: number, stripSeconds: number, audioSeconds: number, loop: boolean): WaveformColumn[] {
    ...
    for (let column = 0; column < columns; column += 1) {
        ...
        const from = waveformPointAt((column / columns) * stripSeconds, audioSeconds, loop, points);
        let to = waveformPointAt(((column + 1) / columns) * stripSeconds, audioSeconds, loop, points) + 1;
