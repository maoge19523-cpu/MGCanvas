...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(function CanvasResourceMentionTextarea(
    ...
    const menu =
        mention && candidates.length && textareaRef.current ? (
            <MentionMenu textareaRef={textareaRef} caretIndex={mention.start} references={candidates} activeIndex={Math.min(activeIndex, candidates.length - 1)} theme={theme} onSelect={insertReference} />
        ) : null;
...
function MentionMenu({
    ...
    const caretRect = new DOMRect(caretLeft, caretTop, 0, lineHeight);
    ...
    return (
        <CanvasNodeAnchoredPopup
            ...
            panelRef={panelRef}
            placement={showAbove ? "topLeft" : "bottomLeft"}
            width={menuWidth}
            maxHeight={maxMenuHeight}
            gap={gap}
            popupProps={{ "data-canvas-resource-mention-menu": "true" } as Record<string, string>}
            className="z-[120] overflow-y-auto rounded-xl border p-1 shadow-2xl backdrop-blur-md"
