...
后续怎么补自动化（建议方向）：① 把「松手才生效」做成用例——拖动过程只允许改 DOM、`pointerup` 才提交，用合成指针事件断言拖动中项目数据逐字未变；② 吸附解析（阈值换算、优先级、快拖抑制）继续以纯函数边界用例覆盖，补上极端缩放与越界夹取；③ 用桩时钟（假的 `AudioContext`）跑长片对齐与合成面板连播；④ 导出用一段固定短素材，在 CI 里跑一次真实 FFmpeg 冒烟并读回产物的时长与音轨；⑤ 主题对比度与导引线可见性用取色断言替代肉眼检查。
