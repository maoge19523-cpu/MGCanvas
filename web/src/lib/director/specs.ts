...
2. 每个镜头都要写清：构图、主体、环境、动作、运镜、声音，以及引用内容出现的准确时刻。
3. 运镜写成「运动类型 + 幅度 + 速度」，例如 slow push-in with small amplitude。
4. 有角色说话时标明说话人；对白、歌词、画面里可见的文字保留原语言，不要翻译。
5. 只写声音提示中真正会被听到的内容，不要写剧情简介或创作说明。`;
...
const SEEDANCE = `目标格式：Seedance 2.0，中文提示词，严格按下面的顺序写完每一节，不许跳节、不许换序：

1. 句柄声明：每行一个 @image1 @image2 …，格式为「@image1 (名称) — 外观描述」。角色句柄要写身高体型、发型、面部特征、从头到脚的服装与随身道具；场景句柄写空间与视觉锚点。
2. 通用警示：依次写 ⚠️空间布局、⚠️对白规则、⚠️本视频严格只有N个镜头 三条。
3. 镜头块：【镜头1】【镜头2】…，每个镜头块内写景别与角度、主体与动作、运镜、光线、表演与情绪。
4. 风格块：以「风格：」开头。
5. 环境活动：以「环境活动：」开头。
6. 收尾行：形如「15秒。21:9。」，写总时长与画幅。

硬性要求：
1. 句柄编号在每个提示词内从 1 重新开始。
2. 镜头数量必须与要求的数量一致，并在警示里写明。
3. 不要在同一个提示词里混用 H3 的字段语法。`;
...
export function buildDirectorSystemPrompt(mode: DirectorMode, target: DirectorTarget): string {
    ...
    if (target === "h3") {
        ...
        parts.push(SEEDANCE);
        parts.push("当前目标：Seedance 2.0。用户上传的参考图按顺序映射为 @image1、@image2 …，并在镜头块里用句柄引用。");
    }
