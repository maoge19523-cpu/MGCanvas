 ...
 */
import { collageLayerExtent, type CollageLayout, type CollageSize } from "./collage-layout";
...
export function drawCollageScene(ctx: CanvasRenderingContext2D, params: CollageSceneParams) {
    const { order, layout, images, sources, scene, scale, selection, selectionColor = "#2f80ff" } = params;
    ctx.save();
    ...
    if (selected && selectedSource) {
        const { halfWidth, halfHeight } = collageLayerExtent(selectedSource, selected);
        ctx.save();
