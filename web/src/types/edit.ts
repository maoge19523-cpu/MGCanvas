...
export type EditAudioTrack = {
    ...
    start?: number;
    /**
     * **素材内的入点**（秒）：这条轨从素材的第几秒开始取（拖音轨条**左端**改的就是它）。
     * 缺省（undefined）= 0 = 从素材开头取，与改动前逐字一致（导出侧不出现 atrim=start，不需要任何迁移）。
     *
     * 与 `start` 的分工（两者语义不同，别混）：`start` 是这条轨在**成片时间轴**上的起点，
     * 入点是这条轨在**素材内部**的起点。裁掉开头**不改 `start`**——音轨仍从同一时刻混进成片，
     * 只是内容改从素材的更后面开始，于是整条轨变短、右端在成片上提前结束
     * （与视频片段拖左端裁剪完全同形：片段在成片里的位置由前序时长决定，裁头只让它变短）。
     */
    sourceStart?: number;
    /**
     * **素材内的出点**（秒），0 / 缺省表示到素材末尾——与 EditClip.end 同一口径。
     * 拖音轨条**右端**改的就是它：音轨结尾被砍掉，左端（`start`）不动。
     */
    sourceEnd?: number;
