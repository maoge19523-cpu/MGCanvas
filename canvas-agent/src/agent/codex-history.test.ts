...
test("Codex 历史省略命令时使用补充事件恢复完整命令卡片", () => {
    const messages = threadMessages({
        ...
        turnId: "turn-1",
        itemId: "command-1",
        item: { id: "command-1", type: "command_execution", command: "Get-Location", status: "completed", exitCode: 0, aggregatedOutput: "D:\\MGCanvas" },
    }], turns: [] });
...
test("标准历史同时保留损坏临时条目和稳定条目时移除损坏副本", () => {
    const cleanText = "目前能排除“网页没开”和“MGCanvas Agent 没连”：前端和 Agent 均正常。";
    const messages = threadMessages({
        id: "thread-1",
        turns: [{ id: "turn-1", status: "completed", items: [
            { id: "user-1", type: "userMessage", content: [{ type: "text", text: "检查连接" }] },
            { id: "item-38", type: "agentMessage", text: "目前能排除“网页没开”和“MGCanvas Agent 没连��：前端和 Agent 均正常。" },
...
test("标准历史条目稀疏时按字段补全补充事件", () => {
    const messages = threadMessages({
        ...
        itemId: "command-1",
        sequence: 1,
        item: { id: "command-1", type: "command_execution", command: "补充命令", cwd: "D:\\MGCanvas", aggregatedOutput: "输出", exitCode: 0 },
    }], turns: [] });
    ...
    assert.deepEqual(command?.detail, {
        kind: "command",
        status: "completed",
        rows: [{ label: "工作目录", value: "D:\\MGCanvas" }, { label: "退出状态", value: "0" }],
        output: "输出",
    });
});
...
test("补充事件写入本地 JSON 后可在 Agent 重启后恢复", async (context) => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "mgcanvas-history-"));
...
test("标准历史尚未物化 turn 时从本地终态事件恢复完整对话", async (context) => {
    ...
    const file = path.join(directory, "codex-event-history.json");
    const history = new CodexEventHistory(file);
    await history.record({ threadId: "thread-1", turnId: "turn-1", itemId: "command-1", sequence: 1, item: { id: "command-1", type: "command_execution", command: "Get-Location", status: "completed", exitCode: 0, aggregatedOutput: "D:\\MGCanvas" } });
...
test("补充事件更新时保留已有字段并限制单项输出大小", async (context) => {
    ...
    context.after(() => fs.rm(directory, { recursive: true, force: true }));
    const history = new CodexEventHistory(path.join(directory, "codex-event-history.json"));
    await history.record({ threadId: "thread-1", turnId: "turn-1", itemId: "command-1", sequence: 1, item: { id: "command-1", type: "command_execution", command: "Get-Location", cwd: "D:\\MGCanvas" } });
    ...
    assert.equal(entry.item.cwd, "D:\\MGCanvas");
