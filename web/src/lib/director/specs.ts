...
2. 镜头数量按官方规范由总时长与叙事节奏决定，不固定数量，并在⚠️警示里写明实际镜头数。
3. 不要在同一个提示词里混用 H3 的字段语法。
4. 对白密度同官方规范：中文普通对白 4.0–4.5 字/秒，慢速画外音 2.5–3.0 字/秒，并留出 0.3–0.5 秒建立与 0.3–0.8 秒收尾的时间。
5. 一条提示词里最多 5 个镜头；15 秒包络建议 2–3 个，写满 6 个以上会明显过密。`;
