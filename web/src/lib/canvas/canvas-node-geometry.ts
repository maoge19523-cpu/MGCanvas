...
import { getConnectionHandleAnchor, normalizeConnectionHandles } from "@/lib/canvas/canvas-node-ports";
...
export function getConnectionTargetAnchor(node: CanvasNodeData, current: ConnectionHandle) {
    return getConnectionHandleAnchor(node, { handleType: current.handleType === "source" ? "target" : "source" });
}
...
export function normalizeConnection(firstNodeId: string, secondNodeId: string, nodes: CanvasNodeData[], firstHandleType: "source" | "target", firstPortId?: string, secondPortId?: string) {
    return normalizeConnectionHandles(
        { nodeId: firstNodeId, handleType: firstHandleType, portId: firstPortId },
        { nodeId: secondNodeId, handleType: firstHandleType === "source" ? "target" : "source", portId: secondPortId },
        nodes,
    );
}
