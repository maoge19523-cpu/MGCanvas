...
- 验证功能修复时**必须读回真实产物**，不能拿「命令执行成功」或「代码已改」当结论：图片要看真正生成的文件与节点上落盘的数据（节点 metadata、`data/media-cache` 里的文件），时长/尺寸这类结论要用 ffprobe 或读回文件实测。
- 命令成功但产物没变，等于这个功能不存在。报告「已修复」之前，先确认产物的修改时间或内容确实对应本次改动。
- 修改运行时 JSON 数据文件前**必须先用 `read` 工具或字节级读取把原文件完整读一遍**，确认里面已有什么；只允许在完整原文基础上追加/修改，**严禁凭印象重写整份文件**。范围包括用户数据目录下的 `.mgcanvas/codex-workspaces/site/.agents/memory.json`、`skills/` 下的 `SKILL.md` 与 `agents/openai.yaml`，以及任何放在隐藏目录（以 `.` 开头）下的配置或数据文件。
- 原因：这类文件是**整份覆盖式写入、无 revision 校验**，不先读回就会静默丢掉原有条目。曾向 `memory.json` 追加一条剧本流程记忆，把用户原有偏好条目「画幅默认 16:9，字幕用思源黑体」覆盖丢失，靠备份才恢复。
- 改这类文件之前先备份（同目录 `.bak-<日期>`），汇报时说明原来有什么、改了什么。
- 验证改动时**不要用默认会跳过隐藏目录的搜索工具**：`grep` / ripgrep 默认跳过 `.` 开头的目录，`.mgcanvas` 里的内容搜不到，**「搜不到」不等于「不存在」**（曾据此把「原有条目完整保留」的错误结论写进汇报）。要么用显式完整路径去读，要么用 `read` 工具直接读回文件逐个核对。
- 改完必须重新读回文件，确认：JSON 仍合法、条目数符合预期、原有条目**逐字未变**。
- 中文文件一律用字节级写入或 `write` / `edit` 工具，**禁止用 PowerShell 5.1 的 `Get-Content` / `Set-Content`**（会 GBK 误解码损坏中文）。
- 这与上面「源码已修好但没打包」是同一类教训：只看源码不看产物，就会把「没生效」误判成「已修好」。
...
- 画布上的高频交互——拖动排序、拖动裁剪入点/出点、拖拽缩放、画布平移——**禁止在 `pointermove` 里调用 `onChange` / store 写入 / `setState`**。过程中只用 `ref` 记录目标值（目标位置、目标秒数），在 `pointerup` / `pointercancel` / `pointerleave` 里**一次性提交**。
- 原因：逐帧写入会在秒级产生几十次重渲染，叠加全局订阅即形成渲染风暴，实测会点燃 React #185（`Maximum update depth exceeded`）。更麻烦的是它**只在生产构建抛错**，开发版与浏览器版都不复现，极难定位——曾为此排查十几轮。
- 同类写法一律照此处理：拖动过程不写画布，松手才落盘。需要过程反馈时，用组件内的预览状态或 CSS 过渡，而不是写画布数据。
- 排查这类崩溃时，先用错误边界把 React 原话与组件栈显示在界面上（生产包没有 DevTools），再按栈里的行号直接搜构建产物（`web/dist/assets/app-*.js`）里出现的特征字符串（如 `componentDidUpdate`、`cm-scroller`）判断组件属于哪个库。
...
- 时间轴上**按时间定位的元素**（标尺刻度、播放头、吸附导引线、片段条、音轨波形条）必须共用**同一个可定位宽度**：纵向滚动、内边距、gap 只能加在**包住全部这些元素的同一个容器**上。只在其中一部分外面套滚动容器（例如音轨行自己 `overflow-y-auto`）时，滚动条会从那部分里扣 6~15px，波形条与标尺随即错开，音画对齐这把尺子失效；片段条也不要再用 flex + gap 分配宽度（同理，间隙会吃掉轨道像素），视觉缝只能画在片段内部。这一条与「画布高频交互禁止逐帧写状态」一样已经踩过两次，改动时间线前先看 `web/src/lib/timeline-scale.ts` 的说明和 `web/src/pages/editor/editor-timeline-alignment-ui.test.tsx` 的断言。
- 之后往时间线上加的**任何新元素**（例如接缝上的转场标记）都照同一条办：**绝对定位浮层、内联样式只写 `left`、不写 `width`**，位置取片段条的右边缘（`placements[i].left + width`），不自己写第二套换算。这样片段条 / 波形条 / 标尺刻度 / 播放头的百分比一个像素都不会动。回归断言见 `web/src/pages/editor/editor-transition-seam-ui.test.tsx`（片段条内联样式与行容器类名逐字冻结）。
