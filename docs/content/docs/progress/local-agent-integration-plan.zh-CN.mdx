...
```bash
codex mcp add mgcanvas -- node /path/to/MGCanvas/canvas-agent/dist/index.js mcp
```

网页侧边栏助手另走 MGCanvas Agent 的对话通道。MGCanvas Agent 使用官方 `@openai/codex` CLI 的 `codex app-server --stdio` 启动并续用同一个 thread，启动时会注入 `mgcanvas` MCP 配置并把 `default_tools_approval_mode` 设为 `approve`，避免 Codex 自己的 MCP 审批卡住侧边栏；真正修改画布前由网页侧边栏做二次确认。
...
MGCanvas Agent 调用 Claude Code 时默认允许 `mcp__mgcanvas__*`，避免 print 模式被工具审批卡住；真正修改画布仍由网页侧边栏确认。
...
1. 前端新增本地 Agent 侧边栏。
2. MGCanvas Agent 通过官方 `@openai/codex` CLI 的 `codex app-server --stdio` 接入 Codex，复用同一个 thread，注入 `mgcanvas` MCP，并展示结构化事件流。
