...
const mgCanvasAgentMcp = mgCanvasAgentMcpCommand();
...
export class CodexAppClient {
    ...
    static async start(emit: AgentEmit, onExit: () => void) {
        ...
        await client.request("initialize", { clientInfo: { name: "mgcanvas-agent", title: "MGCanvas Agent", version: VERSION }, capabilities: { experimentalApi: true, requestAttestation: false } });
...
/** 生成 Codex 调用 MGCanvas Agent MCP 的启动命令。 */
function mgCanvasAgentMcpCommand() {
...
function codexConfig(permissionMode: AgentPermissionMode) {
    return { model_reasoning_summary: "auto", ...(permissionMode === "automatic" ? { approvals_reviewer: "auto_review" } : {}), mcp_servers: { mgcanvas: { command: mgCanvasAgentMcp.command, args: mgCanvasAgentMcp.args, default_tools_approval_mode: "approve", startup_timeout_sec: 20, tool_timeout_sec: 90 } } };
}
