...
import { canvasNodeToolbarAnchor } from "@/lib/canvas/canvas-node-toolbar-position";
...
type CanvasNodeHoverToolbarProps = {
    ...
    canvasSetAvailable: boolean;
    onToggleCanvasSet: (node: CanvasNodeData) => void;
...
export function CanvasNodeHoverToolbar({
    ...
    const { left, top } = canvasNodeToolbarAnchor(node, viewport);
    ...
    const imageTools = buildImageToolbarTools(node, { onUpload, onToggleFreeResize, onCrop, onSplit, onUpscale, onAngle, onViewImage, onCopyPrompt: copyImagePrompt, onReversePrompt });
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(hasImage || hasVideo || hasAudio || hasFile
            ? [
                  {
                      id: "download",
                      title: hasFile ? t("common.download") : t(hasAudio ? "canvas.nodeToolbar.downloadAudio" : hasVideo ? "canvas.nodeToolbar.downloadVideo" : "canvas.nodeToolbar.downloadImage"),
    ...
    return (
        <>
            <div
                className="absolute z-[70] flex h-12 -translate-x-1/2 -translate-y-full items-center overflow-visible rounded-[18px] border text-[15px] shadow-[0_12px_32px_rgba(0,0,0,.22)] backdrop-blur-xl transition-[opacity,filter] duration-150 ease-out"
                style={{ left, top, background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item }}
                data-canvas-node-hover-toolbar
            ...
            >
                {canSetCanvasValue ? (
                    <ToolbarAction
                        id="canvasSet"
                        title={t(canvasSetEnabled ? "canvas.objectReferences.unsetHint" : "canvas.objectReferences.setHint")}
                        label={t(canvasSetEnabled ? "canvas.objectReferences.setActive" : "canvas.objectReferences.setAction")}
                        icon={<Boxes className="size-4" />}
                        active={canvasSetEnabled}
                        onClick={() => onToggleCanvasSet(node)}
                ...
                <div className="h-[56vh] min-h-[360px] select-text text-sm" data-canvas-shortcuts-ignore>
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.metadata?.localPath ? <InfoRow label="本地文件" value={node.metadata.localPath} /> : null}
...
function ToolbarAction({ id, title, label, icon, onClick, showLabel, active = false, danger = false }: ToolbarTool & { showLabel: boolean }) {
    const hasText = showLabel && Boolean(label);
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2} color={theme.node.panel} styles={{ root: { color: theme.node.text, boxShadow: "0 8px 24px rgba(0,0,0,.22)", fontSize: 13, fontWeight: 500 } }}>
            <button type="button" className="group relative flex h-12 items-center whitespace-nowrap px-1.5" style={{ color: danger ? "#f87171" : theme.toolbar.item }} onClick={onClick} aria-label={title} data-canvas-node-toolbar-action={id}>
                <span
                    className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition group-hover:bg-black/[0.05] dark:group-hover:bg-white/[0.07]`}
                    style={{ background: active ? theme.toolbar.activeBg : undefined }}
                >
                    {icon}
