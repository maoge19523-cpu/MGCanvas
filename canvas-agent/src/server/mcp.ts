...
import { AGENT_PROMPT, loadConfig, type MGCanvasAgentConfig, VERSION } from "../config.js";

type MGCanvasAgentToolResponse = { ok?: boolean; result?: unknown; error?: string };

/** 启动通过标准输入输出通信的 MCP 服务。 */
export async function startMcpServer() {
    const config = loadConfig(true);
    const server = new McpServer({ name: "mgcanvas-agent", version: VERSION }, { instructions: AGENT_PROMPT });
    toolNames.forEach((name) => registerCanvasTool(server, config, name));
    await server.connect(new StdioServerTransport());
}

/** 向 MCP Server 注册单个 MGCanvas Agent 工具。 */
function registerCanvasTool(server: McpServer, config: MGCanvasAgentConfig, name: ToolName) {
...
/** 将 MCP 工具调用转发到本地 MGCanvas Agent HTTP 服务。 */
async function postCanvasAgentTool(config: MGCanvasAgentConfig, name: ToolName, input: unknown) {
    ...
    const body = (await res.json()) as MGCanvasAgentToolResponse;
