...
MGCanvas 画布节点插件的 **TypeScript SDK**。插件作者只写节点 UI 与逻辑,类型、JSX、运行时桥接、构建全部由 SDK 提供;产物仍是宿主加载器现有契约的 ESM(React external,宿主单例)。

## 提供什么

| 能力 | 说明 |
| --- | --- |
| **完整类型** | `CanvasPlugin` / `CanvasNodeDefinition` / `CanvasNodeContext` / `MGCanvasAgentOp` / `CanvasTheme` / `CanvasNodeData` … 全部有提示 |
...
```tsx
// src/index.tsx
import { definePlugin, useState } from "@mgcanvas/plugin-sdk";
import type { CanvasNodeContentProps } from "@mgcanvas/plugin-sdk";
...
```js
// build.mjs
import { buildPlugin } from "@mgcanvas/plugin-sdk/build";
...
插件 `tsconfig.json` 关键项:`"jsx": "react-jsx"`、`"jsxImportSource": "@mgcanvas/plugin-sdk"`、`"moduleResolution": "bundler"`。

## 设计约束

- **React 单例**:JSX 与 hooks 惰性读取 `globalThis.MGCanvasRuntime.React`(宿主在加载插件前注入),react 全程 external,绝不打包第二份。
