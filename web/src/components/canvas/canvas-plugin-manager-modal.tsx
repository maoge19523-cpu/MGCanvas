...
export function CanvasPluginManagerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    useEffect(() => {
        if (PLUGIN_REGISTRY_URL && open && official.length === 0 && !loadingOfficial && !officialError) void loadOfficial();
    ...
    const officialTab = (
        <div className="space-y-2">
            <div className="flex items-center justify-between">
                ...
                <Button type="text" size="small" icon={<RefreshCw className={`size-4 ${loadingOfficial ? "animate-spin" : ""}`} />} onClick={loadOfficial} disabled={loadingOfficial || !PLUGIN_REGISTRY_URL}>
