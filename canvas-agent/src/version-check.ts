...
export function checkVersions() {
    ...
    logger.info("MGCanvas Agent version", { version: VERSION });
...
/** 查询 Codex 的 npm 版本；MGCanvas Agent 本身仅从当前本地源码构建。 */
async function checkLatestCodexVersion(localCodexVersion: string) {
    try {
        const latestCodex = await npmVersion("@openai/codex");
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Update the MGCanvas Agent source dependency and rebuild locally.`);
